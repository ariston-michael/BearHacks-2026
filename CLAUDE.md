...
Exception: narrow IPC in `src/main/index.ts` and `src/preload/index.ts` is allowed for (1) `voice:executeIntent` (`search_web` / `open_app`), and (2) `test/full-integration` system control: `cursor:*` and `keyboard:shortcut` (nut-js). Otherwise do not touch main, preload, or electron.vite.config.ts.
